<template>
  <div>
    <a class="nav-link px-2" v-b-modal.modal-cadastrar-publicacao-header>
    ...
    <b-modal
      centered
      id="modal-cadastrar-publicacao-header"
      size="lg"
...
<script>
import serviceTwitter from "@/service/serviceTwitter";
...
export default {
  ...
  methods: {
    ...
    submitDadosPublicacao: function() {
      ...
      formData.append("imagem", this.nova_publicacao.imagem);
      ...
      this.$swal
        .fire({
          ...
          text:
            "Ao salvar essa publicação aparecerá no feed dos seus seguidores!",
          icon: "warning",
        ...
        .then((result) => {
          if (result.value) {
            servicePublicacoes
              .criarPublicacoes(formData)
              .then((resposta) => {
                console.log(resposta.data);
                this.publicacaoIdRetornado = resposta.data.id;
                console.log(this.publicacaoIdRetornado);
    ...
    postarTwitter: function() {
      console.log("entrou");

      serviceTwitter.postarTweet(this.publicacaoIdRetornado);

      this.$router.push({ name: "explorar" });
    },
